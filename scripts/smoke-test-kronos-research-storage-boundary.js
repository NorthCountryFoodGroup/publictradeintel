...
for(const file of new Set(files)){if(file.startsWith("scripts/")||modules.some(name=>file===`kronos/${name}.js`))continue;assert.doesNotMatch(fs.readFileSync(path.join(root,file),"utf8"),/require\s*\([^)]*research-(?:canonical|hash|db-schema|corrections|store)["']/,`${file} must not depend on storage foundation`);}
