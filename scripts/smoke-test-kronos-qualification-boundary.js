...
try {
  ...
  for(const file of new Set(files)) {
    if(file.startsWith("scripts/")||/^kronos\/research-qualification-(?:policy|contracts|authority|signer-contracts|signer-verifier|preflight-contracts|preflight-journal|preflight-gate|witness-contracts|witness-state|witness-disk|witness-store|native-proofs|native-signer)\.js$/.test(file))continue;
