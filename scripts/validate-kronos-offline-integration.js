...
const names = [
  ...["contracts", "approval", "denial", "rollback", "fork", "sequence", "epoch", "process", "lost-response", "crash", "dual-rollback", "freshness", "signer", "proof", "provenance", "secrets", "boundary"].map(n => "smoke:kronos-operator-retention-" + n),
  ...["commands", "authority", "toctou", "races", "recovery", "proof", "boundary"].map(n => "smoke:kronos-operator-" + n),
...
if (names.length !== 81 || new Set(names).size !== 81) throw Error("OFFLINE_SUITE_COUNT");
...
console.log("ALL 81 OFFLINE SUITES PASSED");
