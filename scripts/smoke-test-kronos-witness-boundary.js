...
try{
 ...
 for(const file of new Set(files)){if(file.startsWith("scripts/"))continue;const text=fs.readFileSync(path.join(root,file),"utf8");
  ...
  else if(!/^kronos\/research-qualification-(?:native-(?:proofs|signer)|integration-contracts|public-proof|operator-(?:contracts|store|control|proof|cli))\.js$/.test(file)) assert.doesNotMatch(text,/require\s*\([^)]*research-qualification-witness-/);
 }
