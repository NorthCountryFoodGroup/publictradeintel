...
async function main(){
 ...
 assert.equal(qualified.record.qualificationVersion,"KRONOS_PROVIDER_QUALIFICATION_V1");assert.equal(qualified.simulated,true);assert.equal(qualified.productionQualified,false);assert.equal(qualified.status.productionReady,false);assert.equal(qualified.runtimeQualified,false);
 ...
 for(const scenario of ["ignoreConditional","missingVersion","shortRetention","getUnavailable","corrupt","namespace","secret"]){const bad=f.mockS3(),p=createS3Adapter(f.config,{transport:bad,now:()=>Date.parse(f.instant)});
  ...
  if(scenario==="shortRetention")bad.inject("GetObjectRetention",()=>({Retention:{Mode:"COMPLIANCE",RetainUntilDate:new Date(f.instant)}}));
