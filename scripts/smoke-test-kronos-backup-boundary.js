...
for(const file of files.filter(file=>/^kronos\/research-(backup-[a-z0-9-]+|snapshot)\.js$/.test(file))){const s=fs.readFileSync(path.join(root,file),"utf8");assert.doesNotMatch(s,/@aws-sdk|https?:\/\/|\bfetch\s*\(|process\.env|\bsetInterval\s*\(/,file);}
