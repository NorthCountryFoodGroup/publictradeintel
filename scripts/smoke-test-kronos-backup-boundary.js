...
for(const file of new Set(files)){if(file.startsWith("scripts/")||/^kronos\/research-(backup-[a-z0-9-]+|qualification-(?:policy|contracts|authority|signer-contracts|signer-verifier)|snapshot)\.js$/.test(file))continue;assert.doesNotMatch(fs.readFileSync(path.join(root,file),"utf8"),/require\s*\([^)]*research-(?:backup-|snapshot)/,file);}
for(const file of files.filter(file=>/^kronos\/research-(backup-[a-z0-9-]+|qualification-(?:policy|contracts|authority|signer-contracts|signer-verifier)|snapshot)\.js$/.test(file))){const s=fs.readFileSync(path.join(root,file),"utf8");assert.doesNotMatch(s.replace(file==="kronos/research-backup-s3.js"?"@aws-sdk/client-s3":"never-match", "authorized-sdk"),/@aws-sdk|https?:\/\/|\bfetch\s*\(|process\.env|\bsetInterval\s*\(/,file);}
