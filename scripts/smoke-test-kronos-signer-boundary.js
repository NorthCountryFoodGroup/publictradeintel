...
try {
  ...
  for(const file of new Set(files)){
    ...
    else if(!/^kronos\/research-qualification-(?:preflight-(?:contracts|journal|gate)|witness-(?:contracts|state|disk|store)|native-(?:proofs|signer))\.js$/.test(file)) assert.doesNotMatch(source,/require\s*\([^)]*research-qualification-signer-/);
  }
