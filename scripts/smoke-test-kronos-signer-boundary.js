...
try {
  ...
  for(const file of new Set(files)){
    ...
    else if(!/^kronos\/research-qualification-(?:preflight-(?:contracts|journal|gate)|witness-(?:contracts|state|disk|store)|native-(?:proofs|signer)|integration-contracts|public-proof|operator-(?:contracts|store|control|proof|cli|retention|retention-contracts|retention-witness|retention-proof))\.js$/.test(file)) assert.doesNotMatch(source,/require\s*\([^)]*research-qualification-signer-/);
  }
