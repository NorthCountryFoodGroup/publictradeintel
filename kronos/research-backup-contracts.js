...
function safe(value){
 function scrub(v){if(Array.isArray(v))return v.map(scrub);if(v&&typeof v==="object")return Object.fromEntries(Object.entries(v).map(([k,x])=>{if(k==="versionId"){providerVersion(x);return [k,"opaque-provider-version"];}if(k==="environmentId"){check(["production","staging","development","fixture"].includes(x));return ["deploymentIdentity",x];}return [k,scrub(x)];}));return v;}
...
function providerVersion(value){check(typeof value==="string"&&value.length>0&&value!=="null"&&value.isWellFormed()&&Buffer.byteLength(value,"utf8")<=1024&&!/[\x00-\x1f\x7f-\x9f]/u.test(value));return value;}
...
function validateReceipt(r,context){
 ...
 check(r.receiptVersion===VERSIONS.receipt);sameContext(r,context);descriptor(r.descriptor);sameContext(r.descriptor,context);for(const k of ["objectKey","logicalHash","artifactSha256","sizeBytes"])check(r[k]===r.descriptor[k]);for(const k of ["providerType","containerRef","softwareRevision","retentionPolicyVersion"])label(r[k]);check(r.providerETag===null||typeof r.providerETag==="string");safe(r.providerETag);providerVersion(r.versionId);iso(r.uploadedAt);iso(r.retainUntil);check(["GOVERNANCE","COMPLIANCE"].includes(r.retentionMode)&&r.encryptionStatus==="PROVIDER_MANAGED","BACKUP_RETENTION_UNVERIFIED");if(r.verifiedAt!==null){iso(r.verifiedAt);check(r.verifiedAt>=r.uploadedAt&&r.verifiedAt<r.retainUntil&&r.verificationMethod==="EXACT_GET_SHA256");}else check(r.verificationMethod===null);return r;
}
module.exports={VERSIONS,ERRORS,STATES,POLICY,CONFIG_NAMES,failure,check,shape,safe,identity,sameContext,iso,digest,label,providerVersion,prefix,objectKey,slotKey,encodeObject,descriptor,decodeObject,validateReceipt};
