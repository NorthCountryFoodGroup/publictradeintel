...
async function verifyExact(adapter,locator,expected,{now,retainUntil,retentionPolicyVersion,softwareRevision,requiredRetention}){
 ...
 // The original offline V1 caller contract requires Compliance. Governance needs an explicit trusted requirement.
 if(requiredRetention!==undefined){require("./research-backup-retention").appropriate(requiredRetention,expected.artifactType);c.check(requiredRetention.minimumRetainUntil===retainUntil&&requiredRetention.policyId===retentionPolicyVersion,"BACKUP_RETENTION_UNVERIFIED");}
 ...
 c.decodeObject(expected,download.bytes);c.check(retention?.versionId===locator.versionId,"BACKUP_VERSION_UNAVAILABLE");c.check(retention.mode===(requiredRetention===undefined?"COMPLIANCE":requiredRetention.mode)&&retention.retainUntil>=retainUntil&&retention.policyVersion===retentionPolicyVersion&&retention.encryptionStatus==="PROVIDER_MANAGED","BACKUP_RETENTION_UNVERIFIED");
 if(requiredRetention!==undefined)require("./research-backup-retention").verify(requiredRetention,{mode:retention.mode,retainUntil:retention.retainUntil,policyHash:retention.policyHash,encryptionStatus:retention.encryptionStatus});
