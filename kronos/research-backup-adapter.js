...
function createAdapter(provider,context){
 ...
 const adapter={capabilities:()=>structuredClone(capabilities),putImmutable(d,bytes){c.descriptor(d);scoped(d);c.decodeObject(d,bytes);return invoke("putImmutable",[structuredClone(d),Buffer.from(bytes)]);},headExact(l){scoped(l);c.label(l.versionId);return invoke("headExact",[structuredClone(l)]);},getExact(l){scoped(l);c.label(l.versionId);return invoke("getExact",[structuredClone(l)]);},describeRetention(l){scoped(l);c.label(l.versionId);return invoke("describeRetention",[structuredClone(l)]);},listPrefix(prefix,cursor=null,limit=100){c.check(prefix===c.prefix(context)||prefix.startsWith(c.prefix(context)));c.check(!prefix.includes("..")&&!prefix.includes("\\")&&Number.isInteger(limit)&&limit>0&&limit<=1000);return invoke("listPrefix",[prefix,cursor,limit]);},verifyExact(l,expected,policy){return require("./research-backup-verification").verifyExact(adapter,l,expected,policy);}};
